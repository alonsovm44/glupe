...
Goal: Support $$ name(param1, param2) { ... }. [DONE]
...
Goal: Allow containers to reference variables. [DONE]
...
[X ] Step 1: Create a struct SemanticNode in glupec.cpp to handle the new types (Variable, Constant, Container).
[X ] Step 2: Refactor the big while loop in processInputWithCache to switch on $ vs $$ vs $: vs $$: vs $CONST:.
[X ] Step 3: Implement resolveVariable(node) function to handle LLM generation or literal extraction for variables.
[X ] Step 4: Update callAI prompts to include the "Context Injection" data.
