...
$systemGcc = Get-Command "g++" -ErrorAction SilentlyContinue
if ($systemGcc) {
    Write-Host " System C++ Compiler detected." -ForegroundColor Green
} elseif (-Not (Test-Path "$gccPath\bin\g++.exe")) {
...
# Verify Compiler
if ($systemGcc) {
    Write-Host "   Compiler verified (System)." -ForegroundColor Green
} elseif (Test-Path "$gccPath\bin\g++.exe") {
    try {
...
$batPath = "$yoriBin;%PATH%"
if (-not $systemGcc) { $batPath = "$mingwBin;$batPath" }
...
if ($currentPath -notlike "*$yoriBin*") { $pathNeedsUpdate = $true }
if (-not $systemGcc -and $currentPath -notlike "*$mingwBin*") { $pathNeedsUpdate = $true }
...
if ($pathNeedsUpdate) {
    ...
    if ($addToPath -eq 'Y' -or $addToPath -eq 'y') {
        ...
        if (-not $systemGcc -and $newPath -notlike "*$mingwBin*") { $newPath += ";$mingwBin" }
    ...
    Write-Host " Yori is already in PATH." -ForegroundColor Green
}
